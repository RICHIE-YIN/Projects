from flask_app import app, render_template, redirect, session, request, flash, url_for
...
def createprofile():
    ...
    return redirect(f"/profile/{data['user_id']}/view")

@app.route('/profile/<int:user_id>/view')
def viewprofile(user_id):
    profile = UserProfile.get_one({"user_id": user_id})
    if not profile:
        flash("Profile not found.")
        referrer_url = request.referrer or url_for('index', _external=True)  # Replace 'index' with your home route function name if it's different
