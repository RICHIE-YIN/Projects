...
def view_conversations():
    ...
    user = User.get_one({'id': session['user_id']})
    conversations = Message.get_conversations(user.id)
    contacts = [User.get_one({'id': conv['contact_id']}) for conv in conversations]
    products = [ProductListing.get_one({'id': conv['product_id']}) for conv in conversations]

    return render_template('conversations.html', user=user, contacts=contacts, products=products)

@app.route('/conversation/product/<int:product_id>/contact/<int:contact_id>', methods=['GET'])
def view_conversation(product_id, contact_id):
    if not 'user_id' in session:
        return redirect('/')

    user = User.get_one({'id': session['user_id']})
    contact = User.get_one({'id': contact_id})
    product = ProductListing.get_one({'id': product_id})
    messages = Message.get_messages_for_product(user.id, contact.id, product.id)

    return render_template('conversation.html', user=user, contact=contact, product=product, messages=messages)
