...
def view_product_messages(product_id):
    ...
    product = ProductListing.get_by_id(product_id)
    recipient = User.get_by_id(product.user_id)
    messages = Message.get_messages_for_product(user.id, recipient.id, product.id)

    return render_template('message_product.html', user=user, product=product, recipient=recipient, messages=messages)
