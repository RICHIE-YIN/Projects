...
#     return render_template('conversation.html', user=user, product=product, recipient=recipient, messages=messages, conversations=conversations)
...
def send_product_message(product_id):
    ...
    return redirect(f'/conversation/{recipient_id}/product/{product_id}')
...
@app.route('/conversation/<int:recipient_id>/product/<int:product_id>', methods=['GET'])
def view_conversation(recipient_id, product_id):
    if not 'user_id' in session:
        return redirect('/')
    ...
    product = ProductListing.get_one({'id': product_id})
    messages = Message.get_messages_for_conversation(user.id, recipient.id, product.id)

    return render_template('conversation.html', user=user, recipient=recipient, product=product, messages=messages)
